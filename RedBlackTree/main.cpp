...
//First deletion function; performs initial deletion step without any fixes. Pretty much just gets rid of the node based on the case
void deletion1(node* head, int thingy, node* &root){
  if(head == NULL){
    cout << "No Tree" << endl;
    return;
  ...
  if(head->getLeftNext() == NULL){
    ...
    Gparent = successor->getParent();
    delete successor;
    deletion2(successorKid, saveColor, root, Gparent);
    return;
  }
  deletion2(x, saveColor, root, Gparent); 
}

//Roots out basic cases to determine doubly black case or not
...
//Use when sorting out doubly black cases
void DoublyBlackCase(node* newHead, node* &root, node* Gparent){
